...
class ResidualBlock(nn.Module):
    def __init__(self, channels: int):
        ...
        self.block = nn.Sequential(
            # extracts and captures important features from the input
            # activation function introduces non-linearity, allowing the network
            # to model complex relationships between the input and output
            ConvolutionalBlock(
                in_channels=channels,out_channels=channels,
                add_activation=True, kernel_size=3, padding=1
            ),
            # focuses on adjusting the dimensions (e.g. number of channels) of the
            # features extracted by the first ConvBlock. The absence of the activation function
            # in the second ConvBlock allows the block to learn the residual information
            ConvolutionalBlock(
                in_channels=channels, out_channels=channels,
                add_activation=True, kernel_size=3, padding=1)
            )
...
class Generator(nn.Module):
    def __int__(self, img_channels: int, num_features: int=64, num_residuals: int=6,
                ):
        """
...
class Discriminator(nn.Module):
    def __int__(self, in_channels=3, features=[64, 128, 256, 512]):
        super().__init__()
        self.initial_layer = nn.Sequential(
            nn.Conv2d(
                in_channels,
        ...
        # After the last layer, we apply convolution to produce a 1-dim output
        layers.append(
            nn.Conv2d(
                in_channels,
                1,
...
# /Users/stuckejana/PycharmProjects/pythonProject6
...
def train_fn(
    ...
    for idx, (zebra, horse) in enumerate(loop):
        ...
        if idx % 200 == 0:
            save_image(fake_horse * 0.5 + 0.5, f"output/horse_{idx}.png")
            save_image(fake_horse * 0.5 + 0.5, f"output/zebra_{idx}.png")
