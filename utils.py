...
###############################################
"""
As per the paper - To reduce model oscillation, we update the discriminator using a history of generated images
rather than the ones produced bz the latest generators. We keep an image buffer that stores the 50 previously created 
images.
And here the followed - https://arvix.org/pdf/1612.07828.pdf

This is another strategy used to stabilise the CycleGAN Training
Replay buffer is used to train the discriminator. Generated images are added to the replay buffer and sampled from it.
The replay buffer returns the newly added image with a probabilty of 0.5
        ...
        for element in data.data:
            ...
            if len(self.data) < self.max_size:
                ...
                if random.uniform(0,1) > 0.5:
                    ...
                    # Otherwise, it sends an older generated image and replaces the older image with the newly
                    # generated image.
                    to_return.append(element)
...
class LambdaLR:
    def __init__(self, n_epochs, offset, decay_start_epoch):
        ...
        ) > 0, "Decay must start before the training sesssion ends!"
        self.n_epochs = n_epochs
        self.offset = offset
        self.decay_start_epoch = decay_start_epoch

    def step(self, epoch):
        # Below line checks whether the current epoch has exceeded the decaz epoch(which is 100)
        # e.g. if current epoch is 80 then max (0, 80 - 100) will be 0
        # i.e. then entire numerator will be 0 - so 1 - 0 is 1
        # i.e. the orginial LR remains as it is.
        return 1.0 - max(0, epoch + self.offset - self.decay_start_epoch) / (self.n_epochs - self.decay_start_epoch)

###############################################
# Initialise convolutional layer weights to N(0, 0.02)
###############################################
