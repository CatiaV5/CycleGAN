...
###############################################
# Methods for Image Visualisation
###############################################
...
PyTorch modules processing image data expect tensors in the format C x H x W.
Whereas PILLow and Matplotlib expect image arrazs in the format H x W x C
so to use them with matplotlib you need to reshape it to put the channels as the 
last dimension:
...
def save_img_samples(batches_done):
    """ Saves the generated sample from the test set"""
    ...
    real_A = Variable(imgs["A"].type)
    fake_B = Gen_AB(real_A)
    real_B = Variable(imgs["B"].type)
...
# Creating criterion object (Loss Function) that will measure the error between the predicted and the target
...
###############################################
lr_scheduler_G = torch.optim.lr_scheduler.LambdaLR(optimizer_G, lr_lambda=LambdaLR(hp.n_epoch,
                                                                                   hp.decaz_start_epoch).step)
lr_scheduler_Disc_A = torch.optim.lr_scheduler.LambdaLR(optimizer_Disc_A, lr_lambda=LambdaLR(hp.n_epoch,
                                                                                   hp.decaz_start_epoch).step)
lr_scheduler_Disc_B = torch.optim.lr_scheduler.LambdaLR(optimizer_Disc_B, lr_lambda=LambdaLR(hp.n_epoch,
                                                                                   hp.decaz_start_epoch).step)
...
def train(
    ...
    for epoch in range(hp.epoch, n_epochs):
        for i, batch in enumerate(train_datalaoder):
            ...
            """
            PyTorch stores gradients in a mutable data structure. So we need to set it to a clean state before
            we use it. Otherwise, it will have old information from the previous iterations
            """
            ...
            Compute the cycle consistency loss by comparing the reconstructed reconstructed_A images with real
            real_A images of domain A. Lambda for cycle loss is 10.0 Penalizing 10 times and forcing to learn the 
            translation
            """
            ...
            Backward cycle loss: lambda * ||G_AtoB(G_BtoA(B)) - B || (Equation 2 in the paper)
            Compute the cycle consistency loss by comparing the reconstructed reconstructed_B images with
            real real_B images of domain B. Lambda for cycle loss is 10.0. Penalising 10 times and forcing to learn the translation
            """
            ...
            Also Equation I the code representation of the equation) and perform backpropagation with optimisation
            """
            ...
            And I divide by 2 because as per Paper - " we divide the objective by 2 while optimising D,
            which slows down the rate ar which D learns, relative to the rate of G" 
            """
            ...
            batches_done = epoch * len(train_datalaoder) + i

            batches_left = n_epochs * len(train_datalaoder) - batches_done
            ...
            # If at sameple interval save image
